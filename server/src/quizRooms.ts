...
export class QuizRoom {
    ...
    if(this.phase === "question"){
      if(!this.answers.has(playerId)){
        ...
        if (choiceIndex === question.correctIndex) {
          ...
          this.scores.set(playerId, current + 1000 + Math.round(500 * (this.remaining / question.timerSec)))
          }
  ...
  private getPlayerWsList(): WebSocket[] {
    ...
    const playersList= Array.from(this.players.values()).map(p => p.ws)
    return playersList
  }
  ...
  private broadcastQuestion(): void {
    ...
    const question = this.questions[this.currentQuestionIndex]
    const { correctIndex, ...questionSansReponse } = question
    this.broadcastToAll({ type: 'question',  question: questionSansReponse, index: this.currentQuestionIndex, total: this.questions.length })
  }
  ...
  private broadcastResults(): void {
    ...
    const distribution = question.choices.map((_, i) =>Array.from(this.answers.values()).filter(a => a === i).length)
    this.broadcastToAll({ type: 'results', correctIndex: question.correctIndex, distribution, scores: scores })
  }
...
//Tdo en plus perso : fonction pour annuler le timer a appler partout 